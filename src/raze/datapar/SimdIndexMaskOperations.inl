...
constexpr uint8 __simd_index_mask_operations<_Derived_>::count_leading_zero_bits() const noexcept {
	...
	if constexpr (__has_avx2_support_v<__base::__isa()>)
		return (math::__lzcnt_clz(__integer) - __unused_bits) / __divisor();
	else
		return (math::__bsr_clz(__integer) - __unused_bits) / __divisor();
}

template <class _Derived_>
constexpr uint8 __simd_index_mask_operations<_Derived_>::count_trailing_one_bits() const noexcept {
	const auto __integer = __base::__to_int();
	using _MaskType = decltype(__integer);
	...
	if constexpr (__has_avx2_support_v<__base::__isa()>)
		return math::__tzcnt_ctz(static_cast<std::make_unsigned_t<_MaskType>>(~__integer)) / __divisor();
	else
		return math::__bsf_ctz(static_cast<std::make_unsigned_t<_MaskType>>(~__integer)) / __divisor();
}

template <class _Derived_>
constexpr uint8 __simd_index_mask_operations<_Derived_>::count_leading_one_bits() const noexcept {
	const auto __integer = __base::__to_int();
	using _MaskType = decltype(__integer);
	...
	if constexpr (__has_avx2_support_v<__base::__isa()>)
		return (math::__lzcnt_clz(static_cast<std::make_unsigned_t<_MaskType>>(~__integer)) - __unused_bits) / __divisor();
	else
		return (math::__bsr_clz(static_cast<std::make_unsigned_t<_MaskType>>(~__integer)) - __unused_bits) / __divisor();
}
