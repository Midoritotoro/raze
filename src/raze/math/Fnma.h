...
struct _Configurable_fnma: raze::options::conditional_callable<_Configurable_fnma, _Options_> {
    ...
    static raze_always_inline auto deferred_call(auto __options, const _Type_& __x, const _Type_& __y, const _Type_& __z) noexcept {
        ...
        if constexpr (!options::concepts::same_as<_Mask_, options::unknown_key>) {
            ...
            if constexpr (_Mask_::has_alternative)
                return vx::_Fnma<arch::ISA::SSE2, _Type_>()(__x, __y, __z, __mask, __condition.alternative());
            else
                return vx::_Fnma<arch::ISA::SSE2, _Type_>()(__x, __y, __z, __mask);
        }
        else return vx::_Fnma<arch::ISA::SSE2, _Type_>()(__x, __y, __z);
    }
    ...
    static raze_always_inline auto deferred_call(auto __options, const _Type_& __x, const _Type_& __y, const _Type_& __z) noexcept {
        ...
        auto __chunk_op = [&] <class _Chunk, class ... _Args_> (_Chunk& __chunk, _Args_&& ... __args) raze_always_inline_lambda {
            __chunk = vx::_Fnma<_Abi_::isa, _Value_>()(vx::__storage_unwrap(__chunk), vx::__storage_unwrap<_Args_>(__args)...);
        };
