...
consteval auto make_rotate_right_shuffle_table_() noexcept {
    ...
    for (sizetype shift = 0; shift < num_shifts; ++shift) {
        ...
        for (sizetype idx = 0; idx < num_indices; ++idx) {
            ...
            sizetype src_elem = (dst_elem + num_elements - actual_shift) % num_elements;
...
raze_always_inline auto make_rotate_right_idx_(V, i32 sh) noexcept {
...
raze_always_inline V rotate_right_fallback_(const V& x, Int sh) noexcept {
...
raze_always_inline pattern_vector_t<Pattern> rotate_right_(const pattern_vector_t<Pattern>& x, Pattern p) noexcept {
    using Ret = decltype(generic_shuffle_(x, p));
    if constexpr (is_fallback<Ret> || !native<pattern_vector_t<Pattern>>) return rotate_right_fallback_(x, get_rotate_right_shift(p));
    else return generic_shuffle_(x, p);
}

template <simd_type V>
raze_always_inline V rotate_right_(const V& x, i32 sh) noexcept {
    using Abi = abi_t<V>;
    ...
    if constexpr (native<V>) {
        ...
        if constexpr (is_fallback<Ret>) return rotate_right_fallback_(x, sh);
            ...
            const auto rotate_indices = make_rotate_right_idx_<Abi::isa,
        ...
        return rotate_right_fallback_(x, sh);
    }
}
