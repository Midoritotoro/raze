...
raze_nodiscard raze_always_inline auto zmm_broadcast_low(V x) noexcept {
	...
	return _Broadcast<ISA, __m512i>()(extract_<ISA, T>(x, std::integral_constant<sizetype, 0>{}));
}
	...
	requires(__i >= 0 && __i < (sizeof(_Tp_) / sizeof(_Type_)))
{
	...
	if constexpr (sizeof(_Tp_) == 16) {
		if constexpr (sizeof(_Type_) == 8) {
			if constexpr (__i == 0) return as<_Tp_>(_mm_shuffle_pd(as<__m128d>(__x), as<__m128d>(__x), 0));
			else return as<_Tp_>(_mm_shuffle_pd(as<__m128d>(__x), as<__m128d>(__x), 0x03));
		}
		else if constexpr (sizeof(_Type_) == 4) return as<_Tp_>(_mm_shuffle_epi32(as<__m128i>(__x), __broadcast_pshufd_index(__i)));
		else if constexpr (sizeof(_Type_) == 2 && has_avx2<_ISA_> && __i == 0) return as<_Tp_>(_mm_broadcastw_epi16(as<__m128i>(__x)));
		else if constexpr (sizeof(_Type_) == 1) {
			if constexpr (has_avx2<_ISA_> && __i == 0) return as<_Tp_>(_mm_broadcastb_epi8(as<__m128i>(__x)));
			else if constexpr (!has_ssse3<_ISA_>) {
				__m128i __combined;

				if constexpr (__i < 8) __combined = _mm_unpacklo_epi8(as<__m128i>(__x), as<__m128i>(__x));
				else __combined = _mm_unpackhi_epi8(as<__m128i>(__x), as<__m128i>(__x));

				return as<_Tp_>(__generic_shuffle_native<_ISA_, i16>(__combined, make_splat_pattern<simd<i16, runtime_abi<_ISA_, 8>>, __i % 8>{}));
			}
		}
		...
		if constexpr (__i == 0 && has_avx2<_ISA_>) {
			if constexpr (sizeof(_Type_) == 8)		return as<_Tp_>(_mm256_broadcastq_epi64(as<__m128i>(__x)));
			else if constexpr (sizeof(_Type_) == 4) return as<_Tp_>(_mm256_broadcastd_epi32(as<__m128i>(__x)));
			else if constexpr (sizeof(_Type_) == 2) return as<_Tp_>(_mm256_broadcastw_epi16(as<__m128i>(__x)));
			else if constexpr (sizeof(_Type_) == 1) return as<_Tp_>(_mm256_broadcastb_epi8(as<__m128i>(__x)));
		}
			...
			if constexpr (__i < (__size / 2)) {
				const auto __half = __splat_native<_ISA_, _Type_>(as<__m128i>(__x), __index);
				return as<_Tp_>(_mm256_insertf128_si256(as<__m256i>(__half), as<__m128i>(__half), 1));
			}
			else {
				const auto __half = _mm256_permute_ps(as<__m256>(__x), __broadcast_pshufd_index(std::integral_constant<sizetype, __i % (__size / 2)>{}));
				return as<_Tp_>(_mm256_permute2f128_si256(as<__m256i>(__half), as<__m256i>(__half), 0x31));
			}
		}
		else {
			__m128i __half;

			if constexpr (__i < (__size / 2)) __half = __splat_native<_ISA_, _Type_>(as<__m128i>(__x), __index);
			else __half = __splat_native<_ISA_, _Type_>(_mm256_extractf128_si256(as<__m256i>(__x), 1), __index);

			return as<_Tp_>(_mm256_insertf128_si256(as<__m256i>(__half), as<__m128i>(__half), 1));
		}
	}
	else if constexpr (sizeof(_Tp_) == 64) {
		if constexpr (__i == 0) return as<_Tp_>(__zmm_broadcast_low<_ISA_, _Type_>(__x));
		else {
			constexpr auto __index = std::integral_constant<sizetype, __i % (__size / 4)>{};
			constexpr auto __lane = __i / (__size / 4);

			return as<_Tp_>(__zmm_broadcast_low<_ISA_, _Type_>(__splat_native<_ISA_, _Type_>(
				_mm512_extracti32x4_epi32(as<__m512i>(__x), __lane), __index)));
		}
	}
}
