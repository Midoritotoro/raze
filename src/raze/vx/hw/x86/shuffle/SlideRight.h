	...
	_Slide_right<arch::ISA::SSE3, 128> 
{
		...
		std::integral_constant<uint32, _Shift_> __shift) raze_const_operator noexcept
	{
		if constexpr (__shift >= sizeof(_IntrinType_))
			...
			return __as<_IntrinType_>(_mm_alignr_epi8(
				__as<__m128i>(__left), _mm_setzero_si128(), 16 - __shift));
	}
};
	...
	_Slide_right<arch::ISA::AVX, 256> 
{
		...
		std::integral_constant<uint32, _Shift_> __shift) raze_const_operator noexcept
	{
		...
		if constexpr (__shift >= sizeof(_IntrinType_)) {
			...
			__low_part = __as<__m256i>(__left);
			__high_part = _mm256_inserti128_si256(__high_part, __as<__m128i>(__left), 1);
		}
		...
		if constexpr ((__shift % 16) == 0) 
			return __as<_IntrinType_>(__low_part);

		return __as<_IntrinType_>(_mm256_alignr_epi8(__low_part, __high_part, 16 - (__shift & 0xF)));
	}
};
	...
	_Slide_right<arch::ISA::AVX2, 256> 
{
		...
		std::integral_constant<uint32, _Shift_> __shift) raze_const_operator noexcept
	{
		if constexpr (__shift >= sizeof(_IntrinType_)) {
			...
			return __as<_IntrinType_>(_mm256_alignr_epi32(
				__as<__m256i>(__left), _mm256_setzero_si256(), (8 - (__shift >> 2)) & 7));
		}
		else {
...
struct _Slide_right<arch::ISA::AVX512F, 512> {
		...
		std::integral_constant<uint32, _Shift_> __shift) raze_const_operator noexcept
	{
		...
		if constexpr (__shift == 0) {
			...
			__low_part = __as<__m512i>(__left);
			__high_part = _mm512_maskz_shuffle_i64x2(0xFC, __as<__m512i>(__left),
				__as<__m512i>(__left), 0x90);
		}
		else if constexpr (__shift < 32) {
			__low_part = _mm512_maskz_shuffle_i64x2(0xFC, __as<__m512i>(__left),
				__as<__m512i>(__left), 0x90);
			__high_part = _mm512_maskz_shuffle_i64x2(0xF0, __as<__m512i>(__left),
				__as<__m512i>(__left), 0x40);
		}
		else if constexpr (__shift < 48) {
			__low_part = _mm512_maskz_shuffle_i64x2(0xF0, __as<__m512i>(__left),
				__as<__m512i>(__left), 0x40);
			__high_part = _mm512_maskz_shuffle_i64x2(0xC0, __as<__m512i>(__left),
				__as<__m512i>(__left), 0x00);
		}
		else if constexpr (__shift < 64) {
			__low_part = _mm512_maskz_shuffle_i64x2(0xC0, __as<__m512i>(__left),
				__as<__m512i>(__left), 0x00);
		}
		else {
			return _Broadcast_zeros<arch::ISA::AVX512F, 512, _IntrinType_>()();
		...
		if constexpr ((__shift % 4) == 0) {
			return __as<_IntrinType_>(_mm512_alignr_epi32(
				__as<__m512i>(__left), _mm512_setzero_si512(), (16 - (__shift >> 2)) & 15));
		}

		const auto __low256 = _mm256_alignr_epi8(__as<__m256i>(__low_part),
			__as<__m256i>(__high_part), 16 - (__shift & 0xF));

		const auto __high256 = _mm256_alignr_epi8(_mm512_extracti64x4_epi64(__as<__m512i>(__low_part), 1),
			_mm512_extracti64x4_epi64(__as<__m512i>(__high_part), 1), 16 - (__shift & 0xF));
	...
	_Slide_right<arch::ISA::AVX512F, 512>
{
		...
		std::integral_constant<uint32, _Shift_> __shift) raze_const_operator noexcept
	{
		...
		if constexpr ((__shift % 4) == 0) {
			return __as<_IntrinType_>(_mm512_alignr_epi32(__as<__m512i>(__left),
				_mm512_setzero_si512(), (16 - (__shift >> 2)) & 15));
		}

		return __as<_IntrinType_>(_mm512_alignr_epi8(__low_part, __high_part, 16 - (__shift & 0xF)));
	}
};
