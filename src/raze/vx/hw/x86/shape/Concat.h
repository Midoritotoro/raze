...
struct _Concat {
	...
	raze_nodiscard raze_static_operator raze_always_inline auto operator()(_Tp_ __x, _Tp_ __y) raze_const_operator noexcept {
		if constexpr (__can_concatenate_into_native_register<_ISA_, _Tp_>) {
			if constexpr (sizeof(_Tp_) == 16 && has_avx<_ISA_>) return _mm256_insertf128_si256(__as<__m256i>(__x), __as<__m128i>(__y), 1);
			else if constexpr (sizeof(_Tp_) == 32 && has_avx512f<_ISA_>) return _mm512_inserti32x8(__as<__m512i>(__x), __as<__m256i>(__y), 1);
		}
	}
};
