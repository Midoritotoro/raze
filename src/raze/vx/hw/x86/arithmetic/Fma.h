...
raze_always_inline V fma_(V x, V y, V z) noexcept {
	if constexpr (sizeof(V) == 16 && has_fma3<ISA>) {
		if constexpr (pd<T>) return _mm_fmadd_pd(x, y, z);
		else if constexpr (ps<T>) return _mm_fmadd_ps(x, y, z);
	}
	else if constexpr (sizeof(V) == 32 && has_fma3<ISA>) {
		if constexpr (pd<T>) return _mm256_fmadd_pd(x, y, z);
		else if constexpr (ps<T>) return _mm256_fmadd_ps(x, y, z);
	}
	else if constexpr (sizeof(V) == 64) {
		if constexpr (pd<T>) return _mm512_fmadd_pd(x, y, z);
		else if constexpr (ps<T>) return _mm512_fmadd_ps(x, y, z);
	}
