...
struct _Fnma {
	...
	raze_nodiscard raze_always_inline _Tp_ operator()(_Tp_ __x, _Tp_ __y, _Tp_ __z) const noexcept {
		if constexpr (sizeof(_Tp_) == 16 && __has_fma3_support<_ISA_>) {
			...
			if constexpr (__is_pd_v<_Type_>) return __as<_Tp_>(_mm512_fnmadd_pd(__as<__m512d>(__x), __as<__m512d>(__y), __as<__m512d>(__z)));
			else if constexpr (__is_ps_v<_Type_>) return __as<_Tp_>(_mm512_fnmadd_ps(__as<__m512>(__x), __as<__m512>(__y), __as<__m512>(__z)));
		}

		return _Add<_ISA_, _Type_>()(_Negate<_ISA_, _Type_>()(_Mul<_ISA_, _Type_>()(__x, __y)), __z);
	}
