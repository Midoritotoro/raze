...
raze_always_inline V fms_(V x, V y, V z) noexcept {
	if constexpr (sizeof(V) == 16 && has_fma3<ISA>) {
		if constexpr (pd<T>) return as<V>(_mm_fmsub_pd(as<__m128d>(x), as<__m128d>(y), as<__m128d>(z)));
		else if constexpr (ps<T>) return as<V>(_mm_fmsub_ps(as<__m128>(x), as<__m128>(y), as<__m128>(z)));
	}
	else if constexpr (sizeof(V) == 32 && has_fma3<ISA>) {
		if constexpr (pd<T>) return as<V>(_mm256_fmsub_pd(as<__m256d>(x), as<__m256d>(y), as<__m256d>(z)));
		else if constexpr (ps<T>) return as<V>(_mm256_fmsub_ps(as<__m256>(x), as<__m256>(y), as<__m256>(z)));
	}
	else if constexpr (sizeof(V) == 64) {
		if constexpr (pd<T>) return as<V>(_mm512_fmsub_pd(as<__m512d>(x), as<__m512d>(y), as<__m512d>(z)));
		else if constexpr (ps<T>) return as<V>(_mm512_fmsub_ps(as<__m512>(x), as<__m512>(y), as<__m512>(z)));
	}
