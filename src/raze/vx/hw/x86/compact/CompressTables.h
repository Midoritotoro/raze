...
template <sizetype VerticalSize, sizetype HorizontalSize, sizetype Alignment>
constexpr inline auto make_compress_tables(u32 multiplier, u32 element_group_stride) noexcept {
...
template <sizetype N> 
constexpr inline auto tables_sse = [] {
    static_assert(N == 1 || N == 2 || N == 4 || N == 8, "Unsupported element size for tables_sse");
    return compress_tables<1, 1, 16>();
}();

template <> constexpr inline auto tables_sse<1>  = make_compress_tables<256, 8, 16>(1, 1);
template <> constexpr inline auto tables_sse<2>  = make_compress_tables<256, 16, 16>(2, 2);
template <> constexpr inline auto tables_sse<4>  = make_compress_tables<16, 16, 16>(4, 4);
template <> constexpr inline auto tables_sse<8>  = make_compress_tables<4, 16, 16>(8, 8);

template <sizetype N>
constexpr inline auto tables_avx = [] {
    static_assert(N == 1 || N == 2 || N == 4 || N == 8, "Unsupported element size for tables_avx");
    return compress_tables<1, 1, 16>();
}();

template <> constexpr inline auto tables_avx<4> = make_compress_tables<256, 8, 16>(4, 1);
template <> constexpr inline auto tables_avx<8> = make_compress_tables<16, 8, 16>(8, 2);
