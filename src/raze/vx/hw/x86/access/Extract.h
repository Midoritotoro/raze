...
raze_always_inline _Element_ __extract_first(_Intrin_ __x) noexcept {
	if constexpr (__is_epi64_v<_Element_> || __is_epu64_v<_Element_>) return _mm_cvtsi128_si64x(__as<__m128i>(__x));
	else if constexpr (__is_epi32_v<_Element_> || __is_epu32_v<_Element_>) return _mm_cvtsi128_si32(__as<__m128i>(__x));
	else if constexpr (__is_epi16_v<_Element_> || __is_epu16_v<_Element_>) return _mm_cvtsi128_si32(__as<__m128i>(__x)) & 0xFFFF;
	else if constexpr (__is_epi8_v<_Element_> || __is_epu8_v<_Element_>) return _mm_cvtsi128_si32(__as<__m128i>(__x)) & 0xFF;
	else if constexpr (__is_pd_v<_Element_>) return _mm_cvtsd_f64(__as<__m128d>(__x));
	else if constexpr (__is_ps_v<_Element_>) return _mm_cvtss_f32(__as<__m128>(__x));
}
...
struct _Extract {
		...
		_Tp_ __x, std::integral_constant<sizetype, _I_> __index) raze_const_operator noexcept
	{
		if constexpr (arithmetic_type<_Tp_>) return __x;
			...
			else if constexpr (sizeof(_Element_) == 2) return _mm_extract_epi16(__as<__m128i>(__x), __index);
			else if constexpr (sizeof(_Element_) == 1) return _mm_extract_epi8(__as<__m128i>(__x), __index);
		}
			...
			if constexpr (__is_pd_v<_Element_>) return _mm512_cvtsd_f64(_mm512_maskz_compress_pd(__mask, __as<__m512d>(__x)));
			else if constexpr (__is_ps_v<_Element_>) return _mm512_cvtss_f32(_mm512_maskz_compress_ps(__mask, __as<__m512>(__x)));
			else if constexpr (__is_epi32_v<_Element_> || __is_epu32_v<_Element_>) return _mm512_cvtsi512_si32(_mm512_maskz_compress_epi32(__mask, __as<__m512i>(__x)));
			else if constexpr (__is_epi64_v<_Element_> || __is_epu64_v<_Element_>) return _mm_cvtsi128_si64(__as<__m128i>(_mm512_maskz_compress_epi64(__mask, __as<__m512i>(__x))));
		}
...
struct _Extract_vector {
	...
	raze_static_operator raze_always_inline _ExtractVector_ operator()(_Tp_ __vector, std::integral_constant<sizetype, _I_> __index) raze_const_operator noexcept {
		if constexpr (sizeof(_Tp_) == sizeof(_ExtractVector_)) return __as<_Tp_>(__vector);
		else if constexpr (sizeof(_Tp_) == 32) {
			if constexpr (sizeof(_Tp_) == 16) return _mm256_extracti128_si256(__as<__m256i>(__vector), __index);
		}
		else if constexpr (sizeof(_Tp_) == 64) {
			if constexpr (sizeof(_Tp_) == 16) __vector = _mm512_extracti64x2_epi64(__as<__m512i>(__vector), __index);
			else if constexpr (sizeof(_Tp_) == 32) __vector = _mm512_extracti64x4_epi64(__as<__m512i>(__vector), __index);
		}
	}
};
