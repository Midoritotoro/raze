	...
	raze_nodiscard raze_always_inline auto operator()(_Tp_ __x, _Tp_ __y) const noexcept {
        ...
        if constexpr (sizeof(_Tp_) == 16) {
            if constexpr (__is_pd_v<_Type_>) {
                if constexpr (__avx512vl) return _mm_cmp_pd_mask(__as<__m128d>(__x), __as<__m128d>(__y), _CMP_NEQ_OQ);
                else return __as<_Tp_>(_mm_cmpneq_pd(__as<__m128d>(__x), __as<__m128d>(__y)));
            }
            else if constexpr (__is_ps_v<_Type_>) {
                if constexpr (__avx512vl) return _mm_cmp_ps_mask(__as<__m128>(__x), __as<__m128>(__y), _CMP_NEQ_OQ);
                else return __as<_Tp_>(_mm_cmpneq_ps(__as<__m128>(__x), __as<__m128>(__y)));
            }
            else if constexpr (__avx512vl) {
                if constexpr (__is_epi64_v<_Type_> || __is_epu64_v<_Type_>) return _mm_cmpneq_epi64_mask(__as<__m128i>(__x), __as<__m128i>(__y));
            ...
            if constexpr (__is_pd_v<_Type_>) {
                if constexpr (__avx512vl) return _mm256_cmp_pd_mask(__as<__m256d>(__x), __as<__m256d>(__y), _CMP_NEQ_OQ);
                else return __as<_Tp_>(_mm256_cmp_pd(__as<__m256d>(__x), __as<__m256d>(__y), _CMP_NEQ_OQ));
            }
            else if constexpr (__is_ps_v<_Type_>) {
                if constexpr (__avx512vl) return _mm256_cmp_ps_mask(__as<__m256>(__x), __as<__m256>(__y), _CMP_NEQ_OQ);
                else return __as<_Tp_>(_mm256_cmp_ps(__as<__m256>(__x), __as<__m256>(__y), _CMP_NEQ_OQ));
            }
            else if constexpr (__avx512vl) {
                if constexpr (__is_epi64_v<_Type_> || __is_epu64_v<_Type_>) return _mm256_cmpneq_epi64_mask(__as<__m256i>(__x), __as<__m256i>(__y));
