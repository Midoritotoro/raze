        ...
        if constexpr (sizeof(_Vector_) == 16) {
            if constexpr (sizeof(_Type_) == 8) {
                if constexpr (__avx512vl) {
                    ...
                    static_cast<int8>(__mask)), _mm_setr_epi32(1, 1, 2, 2)), _mm_setzero_si128()));
            }
                ...
                if constexpr (__avx512vl) {
                    ...
                    static_cast<int8>(__mask)), _mm_setr_epi32(1, 2, 4, 8)), _mm_setzero_si128()));
            }
            else if constexpr (sizeof(_Type_) == 2) {
                if constexpr (__avx512vl && __avx512bw) return __as<_Vector_>(_mm_movm_epi16(__mask));
                else return __as<_Vector_>(_mm_cmpgt_epi16(_mm_and_si128(_mm_set1_epi8(static_cast<int8>(__mask)),
            ...
            auto __fallback_avx = [=] () raze_always_inline_lambda {
                ...
                return __as<_VectorType_>(_mm256_insertf128_si256(__as<__m256i>(__low), __high, 1));
            };
