...
raze_always_inline bool all_of_(M x) noexcept {
	if constexpr (intrin_type<M>) {
		constexpr auto size = (sizeof(M) == 64) ? math::max_limit<u64>() : u64((u64(1) << sizeof(M)) - 1);
		return to_bitmask_<ISA, i8>(x) == size;
	}
	else if constexpr (std::is_integral_v<M> && !std::is_same_v<M, bool>) {
		raze_maybe_unused_attribute constexpr auto max_for_bits = (raze_sizeof_in_bits(M) == N)
			? math::max_limit<M>() : M(((M(1) << N) - 1));

		if constexpr (N >= 8 && sizeof(M) == 1 && has_avx512dq<ISA>) return _kortestc_mask8_u8(x, x);
...
raze_always_inline bool all_of_(M x, Mask mask) noexcept {
    if constexpr (std::is_same_v<std::remove_cvref_t<M>, bool>) {
        ...
        if constexpr (N >= 8 && sizeof(M) == 1 && has_avx512dq<ISA>) return _ktestc_mask8_u8(x, mask);
        else if constexpr (sizeof(M) == 2 && has_avx512dq<ISA>) return _ktestc_mask16_u8(x, mask);
        else if constexpr (sizeof(M) == 4 && has_avx512bw<ISA>) return _ktestc_mask32_u8(x, mask);
        else if constexpr (sizeof(M) == 8 && has_avx512bw<ISA>) return _ktestc_mask64_u8(x, mask);
        return (x & mask) == mask;
    }
