...
struct _To_vector {
    ...
    raze_nodiscard raze_always_inline auto operator()(_Mask_ __mask) const noexcept {
        ...
        if constexpr (intrin_type<_Mask_>)
            return as<_Vector_>(__mask);
        else if constexpr (sizeof(_Vector_) == 16) {
            if constexpr (sizeof(_Type_) == 8) {
                if constexpr (__avx512vl) {
                    if constexpr (__avx512dq) return as<_Vector_>(_mm_movm_epi64(__mask));
                    else return as<_Vector_>(_mm512_maskz_mov_epi64(__mask, _All_ones<_ISA_, __m512i>()()));
                }
                else {
                    ...
                    return as<_Vector_>(_mm_cmpgt_epi32(__intersection, _mm_setzero_si128()));
                }
            }
            else if constexpr (sizeof(_Type_) == 4) {
                if constexpr (__avx512vl) {
                    if constexpr (__avx512dq) return as<_Vector_>(_mm_movm_epi32(__mask));
                    else return as<_Vector_>(_mm512_maskz_mov_epi32(__mask, _All_ones<_ISA_, __m512i>()()));
                }
                else return as<_Vector_>(_mm_cmpgt_epi32(_mm_and_si128(_mm_set1_epi8(
                    static_cast<i8>(__mask)), _mm_setr_epi32(1, 2, 4, 8)), _mm_setzero_si128()));
            }
            else if constexpr (sizeof(_Type_) == 2) {
                if constexpr (__avx512vl && __avx512bw) return as<_Vector_>(_mm_movm_epi16(__mask));
                else return as<_Vector_>(_mm_cmpgt_epi16(_mm_and_si128(_mm_set1_epi8(static_cast<i8>(__mask)),
                    _mm_setr_epi32(0x00020001, 0x00080004, 0x00200010, 0x00800040)), _mm_setzero_si128()));
            }
            else if constexpr (sizeof(_Type_) == 1) {
                if constexpr (__avx512vl && __avx512bw) return as<_Vector_>(_mm_movm_epi8(__mask));
                else if constexpr (__ssse3) {
                    const auto __select = _mm_set1_epi64x(0x8040201008040201ull);
                    return as<_Vector_>(_mm_cmpeq_epi8(_mm_and_si128(_mm_shuffle_epi8(
                    ...
                    return as<_Vector_>(_mm_cmpeq_epi8(__combined, _mm_setzero_si128()));
                }
            }
        }
            ...
            auto __fallback_avx = [=] () raze_always_inline_lambda {
                ...
                return as<_Vector_>(_mm256_insertf128_si256(as<__m256i>(__low), as<__m128i>(__high), 1));
            };

            if constexpr (sizeof(_Type_) == 8) {
                if constexpr (__avx512vl) {
                    if constexpr (__avx512dq) return as<_Vector_>(_mm256_movm_epi64(__mask));
                    else return as<_Vector_>(_mm256_maskz_mov_epi64(__mask, _All_ones<_ISA_, __m256i>()()));
                }
                    ...
                    return as<_Vector_>(_mm256_cmpgt_epi64(__selected, _mm256_setzero_si256()));
                }
                else return __fallback_avx();
            }
            else if constexpr (sizeof(_Type_) == 4) {
                if constexpr (__avx512vl) {
                    if constexpr (__avx512dq) return as<_Vector_>(_mm256_movm_epi32(__mask));
                    else return as<_Vector_>(_mm256_maskz_mov_epi64(__mask, _All_ones<_ISA_, __m256i>()()));
                }
                else if constexpr(__avx2) {
                    const auto __vector_mask = _mm256_set1_epi32(static_cast<i32>(__mask));
                    ...
                    return as<_Vector_>(_mm256_cmpgt_epi32(__selected, _mm256_setzero_si256()));
                }
                else return __fallback_avx();
            }
            else if constexpr (sizeof(_Type_) == 2) {
                if constexpr (__avx512vl && __avx512bw) return as<_Vector_>(_mm256_movm_epi16(__mask));
                else if constexpr (__avx2) {
                    ...
                    return as<_Vector_>(_mm256_cmpgt_epi16(_mm256_and_si256(__shuffled, __select), _mm256_setzero_si256()));
                }
                else return __fallback_avx();
            }
            else if constexpr (sizeof(_Type_) == 1) {
                if constexpr (__avx512vl && __avx512bw) return as<_Vector_>(_mm256_movm_epi8(__mask));
                else if constexpr (__avx2) {
                    ...
                    return as<_Vector_>(_mm256_cmpeq_epi8(_mm256_and_si256(__shuffled, __select), __select));
                }
                else return __fallback_avx();
            }
        }
            ...
            if constexpr (sizeof(_Type_) == 8) {
                if constexpr (__avx512dq) return as<_Vector_>(_mm512_movm_epi64(__mask));
                else return as<_Vector_>(_mm512_maskz_mov_epi64(__mask, _All_ones<_ISA_, __m512i>()()));
            }
            else if constexpr (sizeof(_Type_) == 4) {
                if constexpr (__avx512dq) return as<_Vector_>(_mm512_movm_epi32(__mask));
                else return as<_Vector_>(_mm512_maskz_mov_epi32(__mask, _All_ones<_ISA_, __m512i>()()));
            }
            else if constexpr (__avx512bw) {
                if constexpr (sizeof(_Type_) == 2) return as<_Vector_>(_mm512_movm_epi16(__mask));
                else if constexpr (sizeof(_Type_) == 1) return as<_Vector_>(_mm512_movm_epi8(__mask));
            }
            else {
                ...
                return as<_Vector_>(_mm512_inserti64x4(as<__m512i>(__low), as<__m256i>(__high), 1));
            }
        }
    }
};
