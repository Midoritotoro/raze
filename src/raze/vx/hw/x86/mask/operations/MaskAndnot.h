...
struct _Mask_andnot {
	...
	raze_nodiscard raze_static_operator raze_always_inline _Tp_ operator()(_Tp_ __x, _Tp_ __y) raze_const_operator noexcept {
		...
		else if constexpr (sizeof(_Tp_) == 1 && has_avx512dq<_ISA_>) return _kandn_mask8(__x, __y);
		else if constexpr (sizeof(_Tp_) == 2 && has_avx512f<_ISA_>) return _kandn_mask16(__x, __y);
		else if constexpr (sizeof(_Tp_) == 4 && has_avx512bw<_ISA_>) return _kandn_mask32(__x, __y);
		else if constexpr (sizeof(_Tp_) == 8 && has_avx512bw<_ISA_>) return _kandn_mask64(__x, __y);
		else return ~__x & __y;
	}
