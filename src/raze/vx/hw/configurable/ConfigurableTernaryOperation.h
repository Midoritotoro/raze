...
struct _Configurable_ternary_operation_base {
    ...
    struct __impl : public raze::options::strict_elementwise_callable<__impl, _Options_, _SupportedOptions_...> {
        ...
        raze_nodiscard raze_always_inline _Simd_ operator()(
            const _Simd_& __x, 
            const _Simd_& __y,
            const _Simd_& __z) const noexcept
        {
            ...
            const _Simd_&   __z) noexcept 
        {
            ...
            if constexpr (options::concepts::same_as<_Mask_, options::unknown_key>) {
                return __op<_Simd_>(__data(__x), __data(__y), __data(__z));
            }
            else {
                ...
                if constexpr (_Mask_::has_alternative)
                    return __op<_Simd_>(__data(__x), __data(__y), __data(__z), __data(__mask), __data(__condition.alternative()));
                else
                    return __op<_Simd_>(__data(__x), __data(__y), __data(__z), __data(__mask));
            }
        }
