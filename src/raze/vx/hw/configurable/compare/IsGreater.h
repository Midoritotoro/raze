...
struct configurable_is_greater_t: options::conditional_callable<configurable_is_greater_t, Options> {
    ...
    static raze_always_inline auto deferred_call(auto opts, const V& x, const V& y) noexcept {
        ...
        auto chunk_op = [&] <class Chunk, class ... Args> (Chunk& chunk, Args&& ... args) raze_always_inline_lambda {
            chunk = greater_<Abi::isa, Value>(ustorage<Args>(args)...);
        };
...
constexpr inline auto is_greater = options::functor<configurable_is_greater_t>;
