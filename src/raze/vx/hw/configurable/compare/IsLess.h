...
struct _Configurable_is_less: raze::options::strict_elementwise_callable<_Configurable_is_less, _Options_> {
    ...
    static raze_always_inline auto deferred_call(auto __options, const _Type_& __x, const _Type_& __y) noexcept {
        ...
        auto __chunk_op = [&] <class _Chunk, class ... _Args_> (_Chunk& __chunk, _Args_&& ... __args) raze_always_inline_lambda {
            __chunk = _Less<_Abi_::isa, _Value_>()(__storage_unwrap(__chunk), __storage_unwrap<_Args_>(__args)...);
        };
    ...
    using callable_tag_type = _Configurable_is_less;
};
