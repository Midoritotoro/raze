...
struct _Configurable_xor: raze::options::strict_elementwise_callable<_Configurable_xor, _Options_> {
    ...
    static raze_always_inline auto deferred_call(auto __options, const _Type_& __x, const _Type_& __y) noexcept {
        ...
        auto __chunk_op = [&] <class _Chunk, class ... _Args> (_Chunk& __chunk, _Args&&... __args) raze_always_inline_lambda {
            using _Op = std::conditional_t<simd_type<_Type_>, _Xor<_Type_::__isa, _Value_>, _Mask_xor<_Type_::__isa, _Value_>>;
    ...
    using callable_tag_type = _Configurable_xor;
};
