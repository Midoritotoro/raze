...
struct configurable_and_t: options::conditional_callable<configurable_and_t, Options> {
    ...
    static raze_always_inline auto deferred_call(auto opts, const T& x, const T& y) noexcept {
        ...
        auto chunk_op = [&] <class Chunk, class ... Args> (Chunk& chunk, Args&& ... args) raze_always_inline_lambda {
            if constexpr (simd_mask_type<T>) chunk = mask_and_<Abi::isa, Value>(ustorage(chunk), ustorage<Args>(args)...);
            else chunk = bit_and_<Abi::isa, Value>(ustorage(chunk), ustorage<Args>(args)...);
        };
...
constexpr inline auto bit_and = options::functor<configurable_and_t>;
