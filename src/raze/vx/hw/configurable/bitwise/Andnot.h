...
struct configurable_andnot_t: options::conditional_callable<configurable_andnot_t, Options> {
    ...
    static raze_always_inline auto deferred_call(auto opts, const T& x, const T& y) noexcept {
        ...
        auto chunk_op = [&] <class Chunk, class ... Args> (Chunk& chunk, Args&& ... args) raze_always_inline_lambda {
            if constexpr (simd_mask_type<T>) chunk = mask_andnot_<Abi::isa, Value>(ustorage(chunk), ustorage<Args>(args)...);
            else chunk = bit_andnot_<Abi::isa, Value>(ustorage(chunk), ustorage<Args>(args)...);
        };
...
constexpr inline auto bit_andnot = options::functor<configurable_andnot_t>;
