...
struct _Configurable_add: raze::options::conditional_callable<_Configurable_add, _Options_> {
    ...
    static raze_always_inline auto deferred_call(auto __options, const _Type_& __x, const _Type_& __y) noexcept {
        ...
        auto __chunk_op = [&] <class _Chunk, class ... _Args_> (_Chunk& __chunk, _Args_&& ... __args) raze_always_inline_lambda {
            __chunk = _Add<_Abi_::isa, _Value_>()(__storage_unwrap(__chunk), __storage_unwrap<_Args_>(__args)...);
        };
...
constexpr inline auto __add = raze::options::functor<_Configurable_add>;
