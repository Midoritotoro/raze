...
struct configurable_add_t: options::conditional_callable<configurable_add_t, Options> {
    ...
    static raze_always_inline auto deferred_call(auto opts, const V& x, const V& y) noexcept {
        ...
        auto chunk_op = [&] <class Chunk, class ... Args> (Chunk& chunk, Args&& ... args) raze_always_inline_lambda {
            chunk = add_<Abi::isa, Value>(ustorage(chunk), ustorage<Args>(args)...);
        };
...
constexpr inline auto add = options::functor<configurable_add_t>;
