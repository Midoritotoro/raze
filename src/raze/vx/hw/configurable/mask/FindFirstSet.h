...
#  include <src/raze/vx/hw/x86/mask/operations/FindFirstSet.h>
...
struct _Configurable_find_first_set: raze::options::strict_elementwise_callable<_Configurable_find_first_set, _Options_, not_null_option> {
    ...
    static raze_always_inline auto deferred_call(auto __options, const _Type_& __x) noexcept {
        ...
        auto __chunk_op = [&] <class _Chunk, class ... _Args_> (const _Chunk& __chunk, _Args_&& ... __args) raze_always_inline_lambda {
            __index += _Find_first_set<_Abi_::isa, _Chunk::size, _Value_, __unsafe>()(__storage_unwrap(__chunk), __storage_unwrap<_Args_>(__args)...);
    ...
    using callable_tag_type = _Configurable_find_first_set;
};
