...
struct _Configurable_none_of: raze::options::strict_elementwise_callable<_Configurable_none_of, _Options_> {
    ...
    static raze_always_inline auto deferred_call(auto __options, const _Type_& __x) noexcept {
        ...
        auto __chunk_op = [&] <class _Chunk, class ... _Args> (const _Chunk& __chunk, _Args&&... __args) raze_always_inline_lambda {
            return _None_of<_Abi_::isa, _Value_>()(__chunk.data(), std::forward<_Args>(__args)...);
        };
        ...
        if constexpr (!options::concepts::same_as<_Mask_, options::unknown_key>) {
            ...
            if constexpr (_Mask_::has_alternative)
                return __x.__for_each_chunk_all_of(__chunk_op, __data(__mask), __data(__condition.alternative()));
            else
                return __x.__for_each_chunk_all_of(__chunk_op, __data(__mask));
        }
        else {
    ...
    using callable_tag_type = _Configurable_none_of;
};
