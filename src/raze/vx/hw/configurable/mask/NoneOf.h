...
struct _Configurable_none_of: raze::options::strict_elementwise_callable<_Configurable_none_of, _Options_> {
    ...
    static raze_always_inline auto deferred_call(auto __options, const _Type_& __x) noexcept {
        ...
        auto __chunk_op = [&] <class _Chunk, class ... _Args_> (const _Chunk& __chunk, _Args_&& ... __args) raze_always_inline_lambda {
            return _None_of<_Abi_::isa, _Chunk::size, _Value_>()(__storage_unwrap(__chunk), __storage_unwrap<_Args_>(__args)...);
        };
        ...
        if constexpr (!options::concepts::same_as<_Mask_, options::unknown_key>) {
            ...
            if constexpr (_Mask_::has_alternative)
                return __x.__for_each_chunk_all_of(__chunk_op, __mask.__storage().storage(), __condition.alternative().__storage().storage());
            else
