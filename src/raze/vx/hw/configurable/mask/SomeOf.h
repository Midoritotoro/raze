...
struct _Configurable_some_of : raze::options::strict_elementwise_callable<_Configurable_some_of, _Options_> {
    ...
    static raze_always_inline auto deferred_call(auto __options, const _Type_& __x) noexcept {
        ...
        auto __chunk_op = [&] <class _Chunk, class ... _Args> (const _Chunk& __chunk, _Args&&... __args) raze_always_inline_lambda {
            return _Some_of<_Abi_::isa, _Chunk::size, _Value_>()(__chunk.data(), std::forward<_Args>(__args)...);
        };
    ...
    using callable_tag_type = _Configurable_some_of;
};
