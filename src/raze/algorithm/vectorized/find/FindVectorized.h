...
struct __find_vectorized_internal {
        ...
        typename _Simd_::value_type __value) raze_const_operator noexcept
    {
        ...
        if constexpr (_Simd_::is_native_mask_load_supported_v) {
            ...
            const auto __loaded     = datapar::load(datapar::where(__first, __tail_mask));
...
__raze_simd_algorithm_inline _Type_* __find_vectorized(
    ...
    _Type_      __value) noexcept
{
    return const_cast<_Type_*>(datapar::__simd_sized_dispatcher<__find_vectorized_internal, _Type_>()(
