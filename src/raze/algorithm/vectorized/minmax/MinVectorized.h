...
struct __min_vectorized_internal {
        ...
        const void* __last) raze_const_operator noexcept
    {
        ...
        __minimum_values = vx::load<_Simd_>(__first);
        __advance_bytes(__first, sizeof(_Simd_));

        while (__first != __stop_at) {
            __minimum_values = vx::vertical_min(vx::load<_Simd_>(__first), __minimum_values);
            __advance_bytes(__first, sizeof(_Simd_));
        }

        if (__tail_size != 0) {
            if constexpr (_Simd_::is_native_mask_load_supported_v) {
                __minimum_values = vx::vertical_min(__minimum_values, 
                ...
                const auto __horizontal_min = vx::horizontal_min(__minimum_values);

                return (__min < __horizontal_min) ? __min : __horizontal_min;
            }
        }

        return vx::horizontal_min(__minimum_values);
    }
};
...
raze_declare_const_function __raze_simd_algorithm_inline _Type_ __min_vectorized(
    ...
    const void* __last) noexcept
{
    return vx::__simd_sized_dispatcher<__min_vectorized_internal, _Type_>()(
