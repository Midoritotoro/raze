...
raze_constexpr_cxx20 void __verify_range__(const _Type_* const __first, const _Type_* const __last) noexcept {
	raze_debug_assert_log(__first <= __last, "transposed pointer range");
}

template <class _Iterator_, class _Sentinel_>
raze_constexpr_cxx20 void __verify_range__(const _Iterator_& __first, const _Sentinel_& __last) noexcept {
#if !defined(NDEBUG)
