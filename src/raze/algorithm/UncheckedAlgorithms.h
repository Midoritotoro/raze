...
#  define __raze_define_kernel_dispatch() \
	...
	raze_nodiscard constexpr raze_always_inline auto __unchecked_kernel_dispatch(_Size_ __bytes, _Iterator_ __first, _Args_&& ... __args) const noexcept { \
		...
		using _ReturnType_ = decltype(std::declval<_WorkType_>().result()); \
...
#endif // !defined(__raze_define_kernel_dispatch)
