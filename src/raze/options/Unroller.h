...
struct _Unroller {
	...
	struct __impl {
		...
		constexpr raze_always_inline auto operator()(sizetype __aligned_size, sizetype __tail_size, _Function_&& __f, _Args_&& ... __args) const noexcept {
			...
			__f(_Tag_{}, __aligned_size, __tail_size, std::forward<_Args_>(__args)...); // Ignore unrolling
#else
			__f(vx::simd<typename _Tag_::value_type, vx::resize_abi_t<typename _Tag_::abi_type, 
				_Tag_::size() * __unrolling>>{}, __aligned_size, __tail_size, std::forward<_Args_>(__args)...);
