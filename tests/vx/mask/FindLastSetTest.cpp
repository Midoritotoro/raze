...
<class Simd> (rtts::type<Simd>) {
    ...
    auto scalar_find_last = [=](const Mask& m) {
        for (size_t i = N; i > 0; --i)
            if (m[i - 1]) return N - i;
        return N;
    };

    auto scalar_find_last_with_control = [=](const Mask& m, const Mask& control) {
        for (size_t i = N; i > 0; --i)
    ...
    {
        Mask m(false);
        RTTS_EXPECT(raze::vx::find_last_set(m) == N);
    }

    {
        Mask m(true);
        RTTS_EXPECT(raze::vx::find_last_set(m) == 0);
    }

    for (size_t i = 0; i < N; ++i) {
        Mask m(false);
        m[i] = true;
        RTTS_EXPECT(raze::vx::find_last_set(m) == (N - i - 1));
    }
    ...
    for (int iter = 0; iter < 100; ++iter) {
        ...
        RTTS_EXPECT(raze::vx::find_last_set(m) == scalar_find_last(m));
