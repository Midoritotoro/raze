...
RTTS_CASE_TPL("raze::vx::slide_left", rtts::simd::all_simd_infos)
<class Simd> (rtts::type<Simd>) {
    ...
    for (size_t sh = 0; sh < N; ++sh) {
        ...
        for (size_t i = 0; i < N; ++i)
            RTTS_EXPECT(r[i] == ((i + sh < N) ? T(arr[i + sh]) : T(0)));
    }

    [&] <raze::sizetype... I>(std::integer_sequence<raze::sizetype, I...>) {
        ([&] {
            if constexpr (!(N > 16 && (I != N && I != N / 2 && I != 0 && I > 8))) {
                auto r = raze::vx::slide_left(v, std::integral_constant<raze::sizetype, I>{});

                for (size_t i = 0; i < N; ++i)
                    RTTS_EXPECT(r[i] == ((i + I < N) ? T(arr[i + I]) : T(0)));
            }
        }(), ...);
