...
RTTS_CASE_TPL("raze::vx::slide_left_merge", rtts::simd::all_simd_infos)
<class Simd> (rtts::type<Simd>) {
    ...
    for (size_t sh = 0; sh < N; ++sh) {
        auto r = raze::vx::slide_left_merge(v1, v2, sh);

        for (size_t i = 0; i < N; ++i)
            RTTS_EXPECT(r[i] == ((i + sh < N) ? T(arr1[i + sh]) : T(arr2[i + sh - N])));
    }

    [&] <raze::sizetype... I> (std::integer_sequence<raze::sizetype, I...>) {
        ([&] {
            auto r = raze::vx::slide_left_merge(v1, v2, std::integral_constant<raze::sizetype, I>{} );

            for (size_t i = 0; i < N; ++i)
                RTTS_EXPECT(r[i] == ((i + I < N) ? T(arr1[i + I]) : T(arr2[i + I - N])));
        }(), ...);
