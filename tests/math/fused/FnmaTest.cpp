...
RTTS_CASE_TPL("raze::math::fnma", rtts::simd::all_simd_infos)
<class Simd> (rtts::type<Simd>) {
    ...
    const auto r       = raze::math::fnma(a, b, c);
    const auto r_merge = raze::math::fnma[mask, src](a, b, c);
    const auto r_zero  = raze::math::fnma[mask](a, b, c);

    const auto reference_fnma = [](T x, T y, T z) -> T {
        return static_cast<T>((-(x * y)) + z);
    };

    for (size_t i = 0; i < N; ++i) {
        const T expected = reference_fnma(A[i], B[i], C[i]);
        ...
        auto raze_scalar = raze::math::fnma[bool(mask[i]), T(S[i])](A[i], B[i], C[i]);
