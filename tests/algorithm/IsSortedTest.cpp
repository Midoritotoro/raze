...
template <typename T>
void test_is_sorted_random(unsigned seed = 42) {
    RandomGenerator<T> gen(seed);
    ...
    for (int i = 0; i < 10000; ++i) {
        ...
        bool simd_result = raze::algorithm::is_sorted(vec.begin(), vec.end());
        bool std_result = std::ranges::is_sorted(vec.begin(), vec.end());
...
template <typename T>
void test_is_sorted_sorted(unsigned seed = 42) {
    std::mt19937 size_gen(seed + 1);
    std::uniform_int_distribution<int> size_dist(0, 1000);
...
template <typename T>
void test_is_sorted_partially_sorted(unsigned seed = 42) {
    std::mt19937 size_gen(seed + 1);
...
template <typename T>
void test_is_sorted_ranges(unsigned seed = 42) {
    std::mt19937 size_gen(seed + 1);
    std::uniform_int_distribution<int> size_dist(0, 1000);
    ...
    for (int i = 0; i < 1000; ++i) {
        ...
        bool simd_result = raze::algorithm::is_sorted(vec);
        bool std_result = std::ranges::is_sorted(vec);
...
template <typename T>
void test_is_sorted_with_predicate(unsigned seed = 42) {
    std::mt19937 size_gen(seed + 1);
    std::uniform_int_distribution<int> size_dist(0, 1000);
    ...
    for (int i = 0; i < 1000; ++i) {
        ...
        bool simd_result = raze::algorithm::is_sorted(vec.begin(), vec.end(), pred);
        bool std_result = std::ranges::is_sorted(vec.begin(), vec.end(), pred);
...
template <typename T>
void test_is_sorted_with_projection(unsigned seed = 42) {
    std::mt19937 size_gen(seed + 1);
    std::uniform_int_distribution<int> size_dist(0, 1000);
    ...
    for (int i = 0; i < 1000; ++i) {
        ...
        bool simd_result = raze::algorithm::is_sorted(vec.begin(), vec.end(), std::less<T>{}, proj);
        bool std_result = std::ranges::is_sorted(vec.begin(), vec.end(), std::less<T>{}, proj);
...
template <typename T>
void test_is_sorted_edge_cases() {
    {
        std::vector<T> vec;
        bool simd_result = raze::algorithm::is_sorted(vec.begin(), vec.end());
        bool std_result = std::ranges::is_sorted(vec.begin(), vec.end());
    ...
    {
        std::vector<T> vec = {T(1), T(1), T(2), T(2), T(3)};
        bool simd_result = raze::algorithm::is_sorted(vec.begin(), vec.end());
        bool std_result = std::ranges::is_sorted(vec.begin(), vec.end());
        raze_assert(simd_result == std_result);
    }
    ...
    {
        std::vector<T> vec = {T(1), T(2), T(2), T(3), T(3)};
        bool simd_result = raze::algorithm::is_sorted(vec.begin(), vec.end());
        bool std_result = std::ranges::is_sorted(vec.begin(), vec.end());
        raze_assert(simd_result == std_result);
    }
...
template <typename T>
void test_is_sorted_simd_boundaries(unsigned seed = 42) {
...
template <typename T>
void test_is_sorted_with_struct_projection(unsigned seed = 42) {
    for (int i = 0; i < 100; ++i) {
        ...
        bool simd_result = raze::algorithm::is_sorted(vec.begin(), vec.end(), std::less<int>{}, proj);
        bool std_result = std::ranges::is_sorted(vec.begin(), vec.end(), std::less<int>{}, proj);
...
void run_all_tests_for_type() {
    test_is_sorted_random<T>();
    test_is_sorted_sorted<T>();
    test_is_sorted_partially_sorted<T>();
    test_is_sorted_ranges<T>();
    test_is_sorted_with_predicate<T>();
    test_is_sorted_with_projection<T>();
    test_is_sorted_edge_cases<T>();
    test_is_sorted_simd_boundaries<T>();
}
...
int main() {
    ...
    test_is_sorted_with_struct_projection<int>();
