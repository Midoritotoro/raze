...
void check_minmax_element(It first, It last) {
    auto std_res = std::minmax_element(first, last);
    auto simd_res = raze::algorithm::minmax_element(first, last);
    raze_assert(std_res.first == simd_res.first);
    raze_assert(std_res.second == simd_res.second);
}

template <typename It, typename Pred>
void check_minmax_element(It first, It last, Pred pred) {
    auto std_res = std::minmax_element(first, last, pred);
    auto simd_res = raze::algorithm::minmax_element(first, last, pred);
    raze_assert(std_res.first == simd_res.first);
    raze_assert(std_res.second == simd_res.second);
}
...
void run_tests_for_type() {
    ...
    }
    {
        ...
        check_minmax_element(a.begin(), a.end(), pred);
    }
