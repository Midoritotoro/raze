...
template <typename T>
void test_find_end_random(unsigned seed = 42) {
    ...
    for (int i = 0; i < 1000; ++i) {
        ...
        auto simd_result = raze::algorithm::find_end(vec1.begin(), vec1.end(), vec2.begin(), vec2.end());
        auto std_result = std::ranges::find_end(vec1.begin(), vec1.end(), vec2.begin(), vec2.end());
...
template <typename T>
void test_find_end_ranges(unsigned seed = 42) {
    std::mt19937 size_gen(seed + 1);
    std::uniform_int_distribution<int> size_dist(10, 1000);
    std::uniform_int_distribution<int> sub_size_dist(1, 50);
    ...
    for (int i = 0; i < 1000; ++i) {
        ...
        auto simd_result = raze::algorithm::find_end(vec1, vec2);
        auto std_result = std::ranges::find_end(vec1, vec2);
...
template <typename T>
void test_find_end_pred_random(unsigned seed = 42) {
    std::mt19937 size_gen(seed + 1);
    std::uniform_int_distribution<int> size_dist(10, 1000);
    std::uniform_int_distribution<int> sub_size_dist(1, 50);
    ...
    for (int i = 0; i < 1000; ++i) {
        ...
        auto simd_result = raze::algorithm::find_end(vec1.begin(), vec1.end(), vec2.begin(), vec2.end(), pred);
        auto std_result = std::ranges::find_end(vec1.begin(), vec1.end(), vec2.begin(), vec2.end(), pred);
    ...
    for (int i = 0; i < 100; ++i) {
        ...
        auto simd_result = raze::algorithm::find_end(vec1.begin(), vec1.end(), vec2.begin(), vec2.end(), pred_true);
        auto std_result = std::ranges::find_end(vec1.begin(), vec1.end(), vec2.begin(), vec2.end(), pred_true);
        
        raze_assert(simd_result.begin() == std_result.begin());
        raze_assert(simd_result.end() == std_result.end());
        raze_assert(simd_result.begin() == vec1.end() - sub_size);
    }
    ...
    for (int i = 0; i < 100; ++i) {
        ...
        auto simd_result = raze::algorithm::find_end(vec1.begin(), vec1.end(), vec2.begin(), vec2.end(), pred_false);
        auto std_result = std::ranges::find_end(vec1.begin(), vec1.end(), vec2.begin(), vec2.end(), pred_false);
...
template <typename T>
void test_find_end_pred_ranges(unsigned seed = 42) {
    std::mt19937 size_gen(seed + 1);
    std::uniform_int_distribution<int> size_dist(10, 1000);
    std::uniform_int_distribution<int> sub_size_dist(1, 50);
    ...
    for (int i = 0; i < 1000; ++i) {
        ...
        auto simd_result = raze::algorithm::find_end(vec1, vec2, pred);
        auto std_result = std::ranges::find_end(vec1, vec2, pred);
...
template <typename T>
void test_find_end_with_projection(unsigned seed = 42) {
    for (int i = 0; i < 100; ++i) {
        ...
        size_t pos_last = 85;
        for (size_t j = 0; j < 10; ++j) {
            vec1[pos_first + j].x = vec2[j].x;
            vec1[pos_last + j].x = vec2[j].x;
        }
        
        auto proj = [](const Point& p) { return p.x; };
        auto pred = [](int a, int b) { return a == b; };
        
        auto simd_result = raze::algorithm::find_end(vec1.begin(), vec1.end(), vec2.begin(), vec2.end(), pred, proj, proj);
        auto std_result = std::ranges::find_end(vec1.begin(), vec1.end(), vec2.begin(), vec2.end(), pred, proj, proj);
        
        raze_assert(simd_result.begin() == std_result.begin());
        raze_assert(simd_result.end() == std_result.end());
        raze_assert(simd_result.begin() - vec1.begin() == static_cast<std::ptrdiff_t>(pos_last));
        
        auto simd_result2 = raze::algorithm::find_end(vec1, vec2, pred, proj, proj);
        auto std_result2 = std::ranges::find_end(vec1, vec2, pred, proj, proj);
        
        raze_assert(simd_result2.begin() == std_result2.begin());
        raze_assert(simd_result2.end() == std_result2.end());
        raze_assert(simd_result2.begin() - vec1.begin() == static_cast<std::ptrdiff_t>(pos_last));
    }
}

template <typename T>
void test_find_end_edge_cases() {
    {
    ...
    
    {
        ...
        raze_assert(simd_result.begin() == std_result.begin());
        raze_assert(simd_result.end() == std_result.end());
        raze_assert(simd_result.begin() - vec1.begin() == 990);
    }
    ...
    
    {
        ...
        raze_assert(simd_result.begin() == std_result.begin());
        raze_assert(simd_result.end() == std_result.end());
        raze_assert(simd_result.begin() - vec1.begin() == 3);
    }
    
    {
        std::vector<T> vec1 = {T(5), T(6), T(5), T(6), T(7), T(5), T(6)};
        std::vector<T> vec2 = {T(5), T(6)};
        auto simd_result = raze::algorithm::find_end(vec1, vec2);
        auto std_result = std::ranges::find_end(vec1, vec2);
        raze_assert(simd_result.begin() == std_result.begin());
        raze_assert(simd_result.end() == std_result.end());
        raze_assert(simd_result.begin() - vec1.begin() == 5);
    }
    ...
    {
        std::vector<T> vec1 = {T(1), T(2), T(1), T(3), T(1)};
        std::vector<T> vec2 = {T(1)};
        auto simd_result = raze::algorithm::find_end(vec1, vec2);
        auto std_result = std::ranges::find_end(vec1, vec2);
        raze_assert(simd_result.begin() == std_result.begin());
        raze_assert(simd_result.end() == std_result.end());
        raze_assert(simd_result.begin() - vec1.begin() == 4);
    }
...
void test_find_end_multiple_occurrences() {
    ...
    for (int iter = 0; iter < 200; ++iter) {
        size_t main_size = 50 + (iter % 100);
        size_t sub_size = 3 + (iter % 10);
        
        auto vec1 = generate_random_vector<T>(main_size, 42 + iter * 7);
        auto vec2 = generate_random_vector<T>(sub_size, 42 + iter * 13);
        
        std::vector<size_t> positions;
        size_t num_inserts = 2 + (iter % 4);
        
        for (size_t ins = 0; ins < num_inserts; ++ins) {
            std::uniform_int_distribution<size_t> pos_dist(0, main_size - sub_size);
...
void run_all_tests_for_type() {
    test_find_end_random<T>();
    test_find_end_ranges<T>();
    test_find_end_pred_random<T>();
    test_find_end_pred_ranges<T>();
    test_find_end_edge_cases<T>();
    test_find_end_multiple_occurrences<T>();
}
...
int main() {
    ...
    test_find_end_with_projection<int>();
