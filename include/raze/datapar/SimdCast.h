        ...
        && (__is_valid_simd_v<_FromType_> || __is_intrin_type_v<_FromType_>)
{
    ...
    if constexpr (__is_valid_simd_v<_FromType_>)
        return __intrin_bitcast<_ToIntrinType>(static_cast<typename _FromType_::vector_type>(__from));
    else
        return __intrin_bitcast<_ToIntrinType>(__from);
}
        ...
        requires(__is_valid_simd_v<_FromVector_> || __is_intrin_type_v<_FromVector_>)
{
    ...
    if constexpr (__is_valid_simd_v<_FromVector_>)
        return __intrin_bitcast<_ToIntrinType>(static_cast<typename _FromVector_::vector_type>(__from));
    else
        return __intrin_bitcast<_ToIntrinType>(__from);
}
