...
struct _Extreme_elements : _Traits_ {
	...
	struct __vectorized_extreme_elements {
			...
			_Comp_ __comp, Projection __proj) const noexcept requires(vx::simd_type<_Tag_>)
		{
			...
			__seek_iter(__first_extreme_it, __first_extreme);
			__seek_iter(__last_extreme_it, __last_extreme);
	...
	{
		auto __r = __extreme_elements_unchecked(traits::__uiter<_Sentinel_>(std::move(__first)),
			traits::__usent<_Iterator_>(std::move(__last)), traits::fwd_fn(__comp), 
	...
	constexpr raze_always_inline std::pair<std::ranges::borrowed_iterator_t<Range>, std::ranges::borrowed_iterator_t<Range>>
		operator()(Range&& __range, _Comp_ __comp = {}, Projection __proj = {}) const noexcept requires(!constexpr_sized_range<Range>)
	{
		auto __begin = std::ranges::begin(__range);
		auto __r = __extreme_elements_unchecked(traits::__r_uiter<Range>(std::move(__begin)),
			traits::__uend(__range), traits::fwd_fn(__comp), traits::fwd_fn(__proj));
