...
struct _Extreme : _Traits_ {
	...
	struct __impl {
		...
		raze_always_inline raze_nodiscard constexpr bool operator()(_Tag_) noexcept {
			if (_iterator == _sentinel) return true;
				...
				if (std::invoke(_comp, std::invoke(_proj, *_iterator), std::invoke(_proj, _extreme_value))) _extreme_value = *_iterator;
				++_iterator;
				return false;
			}
		}
	...
	struct __vectorized_max_value {
			...
			_Sentinel_ __sentinel, _Comp_ __comp, _Projection_ __proj) const noexcept requires(!vx::simd_type<_Tag_>)
		{
			...
			for (; ++__first != __sentinel;)
				if (__comp(__proj(*__first), __proj(__extreme_value)))
					__extreme_value = *__first;
	...
	{
		return __extreme_unchecked(type_traits::__ranges_unwrap_iterator<_Sentinel_>(std::move(__first)),
	...
	{
		return __extreme_unchecked(type_traits::__unchecked_begin(__range),
...
constexpr inline auto extreme = raze::options::function_with_traits<_Extreme>;
