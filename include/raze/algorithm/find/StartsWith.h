...
struct _Starts_with : _Traits_ {
                 ...
                 std::indirectly_comparable<_Iterator1_, _Iterator2_, Predicate, _Projection1_, _Projection2_>
	{
		return algorithm::mismatch[_Traits_::traits()](std::move(__first1), std::move(__sent1), 
			std::move(__first2), __sent2, traits::fwd_fn(__pred), traits::fwd_fn(__proj1), 
			traits::fwd_fn(__proj2)).in2 == __sent2;
	}
				...
				std::ranges::iterator_t<_Range2_>, Predicate, _Projection1_, _Projection2_>
	{
		...
		return algorithm::mismatch[_Traits_::traits()](std::forward<_Range1_>(__r1),
			std::forward<_Range2_>(__r2), traits::fwd_fn(__pred), traits::fwd_fn(__proj1),
			traits::fwd_fn(__proj2)).in2 == __r2;
	}
};
