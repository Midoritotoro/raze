...
struct _All_of : _Traits_ {
	...
	struct __impl {
			...
			requires(!options::concepts::same_as<_Tag_, vx::scalar_tag>)
		{
			...
			raze_disable_unrolling
			do {
				if (!vx::all_of(_predicate(_proj(raze::vx::load<_Tag_>(__ptr))))) return false;
	...
	{
		return __all_of_unchecked(type_traits::__unchecked_begin(__range), type_traits::__unchecked_end(__range),
			type_traits::__pass_function(__pred), type_traits::__pass_function(__proj), __bytes_distance(__range));
	}
private:
	raze_define_unchecked(_All_of);
