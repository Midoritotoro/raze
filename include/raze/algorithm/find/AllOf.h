...
struct _All_of : _Traits_{
	...
	struct __impl {
		...
		raze_always_inline raze_nodiscard constexpr bool operator()(_Tag_) noexcept {
			...
			else if (!_predicate(_proj(*_iterator))) { _result = false; return true; }
			else {
				++_iterator;
				return false;
			}
	...
	struct __vectorized_all_of {
			...
			_Projection_ __proj) const noexcept requires(!vx::simd_type<_Tag_>)
		{
			for (; __first != __sentinel; ++__first)
				if (!__predicate(__proj(*__first))) return false;
			...
			_Projection_ __proj) const noexcept requires(vx::simd_type<_Tag_>)
		{
			...
			do {
				if (vx::some_of(__predicate(__proj(raze::vx::load<_Tag_>(__ptr))))) return false;
	...
	{
		return __all_of_unchecked(type_traits::__ranges_unwrap_iterator<_Sentinel_>(std::move(__first)),
	...
	{
		return __all_of_unchecked(type_traits::__unchecked_begin(__range),
...
private:
	template <class _Iterator_, class _Sentinel_, class _Predicate_, class _Projection_>
	raze_nodiscard constexpr raze_always_inline bool __all_of_unchecked(
		_Iterator_ __first, _Sentinel_ __last, _Predicate_ __pred, _Projection_ __proj) const noexcept
	{
		...
		{
			if not consteval {
				return vx::__dispatch_sized_impl<__vectorized_all_of, _Value_, bool>(
	...
	raze_nodiscard constexpr raze_always_inline bool __all_of_unchecked(_Iterator_ __first,
		_Sentinel_ __last, _Predicate_ __pred, _Projection_ __proj, std::integral_constant<sizetype, _Size_> __size) const noexcept
	{
			...
			&& vectorizable_projection<_Projection_, _Iterator_>)
		{
			if not consteval {
				...
				return vx::__dispatch_sized_impl<__vectorized_all_of,
