...
struct _Contains_subrange : _Traits_ {
			...
			requires(std::indirectly_comparable<_Iterator1_, _Sentinel1_, Predicate, _Projection1_, _Projection2_>)
	{
		...
		return algorithm::search[_Traits_::traits()](std::move(__first1), std::move(__last1),
			std::move(__first2), std::move(__last2), traits::fwd_fn(__pred), 
			traits::fwd_fn(__proj1), traits::fwd_fn(__proj2)) != __end;
	}
				...
				std::ranges::iterator_t<_Range2_>, Predicate, _Projection1_, _Projection2_>)
	{
		...
		return algorithm::search[_Traits_::traits()](std::forward<_Range1_>(__r1), std::forward<_Range2_>(__r2),
			traits::fwd_fn(__pred), traits::fwd_fn(__proj1), traits::fwd_fn(__proj2)) != __end;
	}
};
