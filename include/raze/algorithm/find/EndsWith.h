...
struct _Ends_with : _Traits_ {
	...
	constexpr raze_always_inline bool operator()(_Iterator1_ __first1,
		...
		Predicate __pred = {}, _Projection1_ __proj1 = {}, _Projection2_ __proj2 = {}) const noexcept
			requires (std::forward_iterator<_Iterator1_> || std::sized_sentinel_for<_Sentinel1_, _Iterator1_>) &&
                 ...
                 std::indirectly_comparable<_Iterator1_, _Iterator2_, Predicate, _Projection1_, _Projection2_>
	{
		const auto __size1 = std::ranges::distance(__first1, __sent1);
		const auto __size2 = std::ranges::distance(__first2, __sent2);

		if (__size1 < __size2) 
			return false;

		std::ranges::advance(__first1, __size1 - __size2);

		return algorithm::equal[_Traits_::traits()](std::move(__first1), std::move(__sent1),
			std::move(__first2), std::move(__sent2), traits::fwd_fn(__pred), 
	...
	{
		return (*this)(std::ranges::begin(__r1), std::ranges::end(__r1), std::ranges::begin(__r2),
			std::ranges::end(__r2), traits::fwd_fn(__pred), traits::fwd_fn(__proj1), traits::fwd_fn(__proj2));
	}
};
