...
struct _Find_end : _Traits_ {
	...
	struct __vectorized_find_end {
			...
			_Predicate_ __predicate, _Projection1_ __proj1, _Projection2_ __proj2) const noexcept
		{
			...
			auto __result = algorithm::search(std::move(__first1), __sentinel1, __first2, __sentinel2, __predicate, __proj1, __proj2);

			if (__result.empty())
				return __result;

			for (;;) {
				auto __new_result = algorithm::search(std::next(__result.begin()), __sentinel1, __first2, __sentinel2, __predicate, __proj1, __proj2);
